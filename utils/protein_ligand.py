...
class PDBProtein(object):
    ...
    def _parse(self):
        ...
        for atom in self._enum_formatted_atom_lines():
            ...
            self.pos.append(np.array([atom['x'], atom['y'], atom['z']], dtype=float))
    ...
    def to_dict_atom(self):
        return {
            'element': np.array(self.element, dtype=float),
            'molecule_name': self.title,
            'pos': np.array(self.pos, dtype=float),
            'is_backbone': np.array(self.is_backbone, dtype=bool),
            'atom_name': self.atom_name,
            'atom_to_aa_type': np.array(self.atom_to_aa_type, dtype=float)
        }

    def to_dict_residue(self):
        return {
            'amino_acid': np.array(self.amino_acid, dtype=float),
            'center_of_mass': np.array(self.center_of_mass, dtype=float),
            'pos_CA': np.array(self.pos_CA, dtype=float),
            'pos_C': np.array(self.pos_C, dtype=float),
            'pos_N': np.array(self.pos_N, dtype=float),
            'pos_O': np.array(self.pos_O, dtype=float),
        }
...
def parse_sdf_file(config,path):
    ...
    smiIndices_input = fetchIndices(smiArr_input,config.model.decoder.smiVoc,config.model.decoder.tgt_len,'input')
    smiIndices_tgt = fetchIndices(smiArr_tgt,config.model.decoder.smiVoc,config.model.decoder.tgt_len,'tgt')
    ...
    feat_mat = np.zeros([rd_num_atoms, len(ATOM_FAMILIES)], dtype=float)
    ...
    element, pos = [], []
    accum_pos = np.array([0.0, 0.0, 0.0], dtype=float)
    accum_mass = 0.0
    ...
    center_of_mass = np.array(accum_pos / accum_mass, dtype=float)
    ...
    edge_index = np.array([row, col], dtype=float)
    edge_type = np.array(edge_type, dtype=float)
    ...
    data = {
        ...
        'weight':weight,
        'tpsa':tpsa,
        'smiIndices_input':smiIndices_input,
        'smiIndices_tgt':smiIndices_tgt
    }
    return data
